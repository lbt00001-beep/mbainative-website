import { normalize, pickVoice, PlaybackGate } from './core.mjs';
...
if(typeof window!=='undefined'&&!customElements.get('mbai-assistant')){
  class PageGuide extends HTMLElement{
    ...
    connectedCallback(){
      ...
      this.onPageHide=()=>this.stop();window.addEventListener('pagehide',this.onPageHide);
      this.onPop=()=>{if(location.pathname!==this.path){this.stop();this.path=location.pathname;this.load();}};window.addEventListener('popstate',this.onPop);
      ...
      this.onNavigate=e=>{const a=e.target.closest?.('a[href]');if(a&&a.origin===location.origin&&a.pathname!==location.pathname)this.stop();};document.addEventListener('click',this.onNavigate);
      ...
      if(!document.getElementById('mbai-guide-highlight-style')){const style=document.createElement('style');style.id='mbai-guide-highlight-style';style.textContent='[data-mbai-narrating]{outline:3px solid #70dcc5!important;outline-offset:7px!important;scroll-margin-top:120px}.mbai-guide-reserved{padding-bottom:var(--mbai-guide-space,80px)!important}@media print{mbai-assistant{display:none!important}.mbai-guide-reserved{padding-bottom:0!important}}';document.head.append(style);}
    ...
    disconnectedCallback(){
      this.stop();this.loadController?.abort();this.resize?.disconnect();for(const url of this.urls.values())URL.revokeObjectURL(url);this.urls.clear();
    ...
    reserve(){document.body.style.setProperty('--mbai-guide-space',Math.ceil(this.getBoundingClientRect().height+30)+'px');}
    expand(value){this.expanded=value;this.ui.panel.hidden=!value;this.ui.launcher.hidden=value;this.reserve();}
    async load(){
      ...
      try{const response=await fetch('/api/assistant/config?path='+encodeURIComponent(this.path),{signal:controller.signal,cache:'no-store'});if(!response.ok)throw Error('guide');const result=await response.json();if(controller.signal.aborted||!this.isConnected)return;
        this.chapters=result.chapters;this.voices=result.voices;this.voiceId=this.voices[0]?.id||'browser';this.ready=true;
    ...
    renderVoices(){
      ...
      const best=pickVoice(local);options.push({id:'browser',label:best?'Navegador · '+best.name:'Navegador · voz española automática'});
    ...
    async play(){
      ...
      this.expand(true);const token=this.gate.next();this.state='loading';this.render();this.ui.caption.textContent=chunk.text;this.ui.caption.scrollTop=0;
    ...
    speakBrowser(text,token){
      ...
      const voices=window.speechSynthesis.getVoices();const voice=this.voiceId.startsWith('browser:')?voices.find(v=>v.voiceURI===this.voiceId.slice(8)):pickVoice(voices);
